...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    let new_ticket = Ticket::new(title.clone(), description, status.clone());
    match new_ticket {
        Ok(ticket) => Ticket {
            title: ticket.title,
        ...
        Err(TicketNewError::InvalidTitle(e)) => panic!("{}", e),
        Err(TicketNewError::InvlaidDescription(e)) => Ticket {
            title,
...
impl Ticket {
    pub fn new(
        ...
        if description.is_empty() {
            return Err(TicketNewError::InvlaidDescription(
